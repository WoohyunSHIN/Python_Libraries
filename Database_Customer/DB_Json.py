...
class Customer:
    ...
    def saveData(self):
        with open('./Python_Study/customer/data.json','wt') as f:
            json.dump(self.custlist,f,indent=4)

    def loadData(self):
        #파일 크기가 0보다 클 경우에  읽어옴.
        #if os.path.getsize('./Python_Study/customer/data.pkl')>0: 
        #파일이 존재할 경우에 읽어옴.
        if os.path.exists('./Python_Study/customer/data.pkl'):    
             with open('./Python_Study/customer/data.json','rt') as f:
